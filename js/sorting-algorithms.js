...
async function initPlay(imgBub, imgBub2, imgForu, imgForu2, imgCube, imgCube2, arr, c) {
  ...
  for(let i = 0; i < arr.n; i++){
    if (c.alg == "bubble") {
      ...
      bub = new Bubble(imgForu, imgForu2, arr.spds[arr.si], sizex, -(sizex*arr.n*0.1)+arr.n-arrSorted.indexOf(arr.nums[i])*sizex, i*sizex+120, arr.n*sizex+40, arr.nums[i], 0);
    } else if (c.alg == "insert") {
